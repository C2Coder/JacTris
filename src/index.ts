...
let savedArray: number[][] = new Array(display.height);

for (let i = 0; i < display.height; i++) {
	savedArray[i] = new Array(display.width).fill(0);
}

let displayArray: number[][] = new Array(display.height);
...
function rotateMatrix(matrix: number[][]): number[][] {
	...
	const rotatedMatrix: number[][] = new Array(cols).fill(0).map(() => new Array(rows).fill(0));
...
function preparePiece(): number[][] {

	while (activePiece.rotation > 3) {
		activePiece.rotation -= 4;
	}
	let out_matrix: number[][] = pieces.table[activePiece.type];
	...
	for (let y = 0; y < p_h; y++) {
		for (let x = 0; x < p_w; x++) {
			...
			if (matrix[y][x] != 0) {
				...
				if (p_x >= display.width || p_x < 0) { undo(); return }
...
function drawPiece(matrix: number[][]) {
